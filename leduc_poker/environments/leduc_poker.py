...
# LEDUC_CARDS = ['J', 'Q', 'K', 'J', 'Q', 'K']  # Suits not relevant for payoff
...
def get_exploitability(agg_hagent, skip_illegal=False, br_type='exact'):
    exploitability = 0.
    ...
    for k,v in br_strat_map.items():
        if br_type == 'approx_br_clean':
            v[:] = np.where(v == np.max(v), 0.75, 0.125)
            if np.sum(v)>1.:
                v[:] = v[:] / np.sum(v[:])

        elif br_type == 'approx_br_rand':
            probs = np.random.uniform(size=(1,3))
...
class LeducPop:
    def __init__(self, env, seed=0, skip_illegal=True):
        ...
        self.pop_size = 3
        self.num_learners = 1
        self.hpop = [self.new_agent(seed=seed + i) for i in range(self.pop_size)]

        # Vector population
        self.vpop = [self.agent2vec(agent) for agent in self.hpop]  # This are just dict of state to prob
        ...
        # Init fixed metagame
        self.nb_fix_pol = self.pop_size-self.num_learners
        self.fixed_metagame = np.empty((self.pop_size, self.pop_size))
        self.fixed_metagame[:] = np.NaN
        self.fixed_metagame[:self.nb_fix_pol, :self.nb_fix_pol] = self.metagame[:self.nb_fix_pol, :self.nb_fix_pol]
    ...
    def new_agent(self, seed=0):
        ...
        for infostate in self.infostates:
            p = r.rand(3)
            hagent[infostate] = p / np.sum(p)
        if self.skip_illegal:
            hagent = self.remove_illegal(hagent)
        return hagent
    ...
    def add_new(self, seed):
        # Create new memeber to pop
        self.hpop.append(self.new_agent(seed=seed))
        self.vpop.append(self.agent2vec(self.hpop[-1]))

        # Fix lower agent
        self.fixed_metagame[self.nb_fix_pol, :self.nb_fix_pol+1] = self.metagame[self.nb_fix_pol, :self.nb_fix_pol+1]
        self.fixed_metagame[:self.nb_fix_pol, self.nb_fix_pol] = self.metagame[:self.nb_fix_pol, self.nb_fix_pol]

        # Increase pop size and fixed pop size
        self.pop_size += 1
        self.nb_fix_pol = self.pop_size-self.num_learners

        # Expand metagame size
        metagame = np.zeros((self.pop_size, self.pop_size))
        metagame[:-1, :-1] = self.metagame
        self.metagame = metagame

        # Expand fixed metagame
        fixed_metagame = np.empty((self.pop_size, self.pop_size))
        fixed_metagame[:] = np.NaN
        fixed_metagame[:-1, :-1] = self.fixed_metagame
        self.fixed_metagame = fixed_metagame

    def get_metagame(self, k=None):
        if k is None:
            for i, hagent1 in enumerate(self.hpop):
                for j, hagent2 in enumerate(self.hpop):
                    val = self.fixed_metagame[i, j]
                    self.metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent1, hagent2)
        ...
        for i, hagent1 in enumerate(self.hpop):
            ...
            for j, hagent2 in enumerate(self.hpop):
                if j >= k:
                    continue
                val = self.fixed_metagame[i, j]
                metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent1, hagent2)
        return metagame

    def get_tmp_metagame(self, hagent_tmp, k):
        metagame = np.zeros((k + 1, k + 1))
        for i, hagent1 in enumerate(self.hpop + [hagent_tmp]):
            if i > k:
                continue
            for j, hagent2 in enumerate(self.hpop + [hagent_tmp]):
                if j > k:
                    continue
                if i == k and j == k:
                    val = self.fixed_metagame[i, j]
                    metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent_tmp, hagent_tmp)
                elif i == k:
                    val = self.fixed_metagame[i, j]
                    metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent_tmp, hagent2)
                elif j == k:
                    val = self.fixed_metagame[i, j]
                    metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent1, hagent_tmp)
                else:
                    val = self.fixed_metagame[i, j]
                    metagame[i, j] = val if not np.isnan(val) else self.get_payoff(hagent1, hagent2)
        return metagame
    ...
    def update(self, br_vagent, k, lr):
        self.vpop[k] = lr * br_vagent + (1 - lr) * self.vpop[k]
        self.hpop[k] = self.vec2agent(self.vpop[k])
        if self.skip_illegal:
            self.hpop[k] = self.remove_illegal(self.hpop[k])
            self.vpop[k] = self.agent2vec(self.hpop[k])
