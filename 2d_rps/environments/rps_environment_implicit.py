...
class GMMAgent_nn(nn.Module):
    def __init__(self, mu, game):
        super(GMMAgent_nn, self).__init__()
        self.mu = mu
        self.game = game
        self.gauss = MyGaussianPDF(mu).to(device_train)
        self.x = (0.01*torch.randn(58, dtype=torch.float)).clone().detach().to(device_train)
        #self.x = nn_logits(self.x)

    def forward(self):
        return self.gauss(nn_logits(self.x))
...
class TorchPop_nn:
    ...
    def agg_agents_agent(self, metanash):
        agg_agent = GMMAgent_nn(self.mus, self.game)
    ...
    def get_exploitability_implicit_train(self, meta_nash, train_iters=100, lr=1.0):
        agg_agent = self.agg_agents_agent(meta_nash)
        exp_best_response_trainer = implicit_best_responder()
        exp, _ = exp_best_response_trainer(agg_agent.x, agg_agent)
        return 2 * exp
...
def get_payoff_nn(agent1, agent2, mus, game, ag_type='logits'):
    if ag_type == 'Agent':
        p = agent1()
        q = agent2()
        ...
        p = gauss(nn_logits(agent1))
        q = gauss(nn_logits(agent2))
    
    return p @ game @ q + 0.5*(p-q).sum()

def nn_logits(weights):
    inp = torch.ones(1,4).to(device_train)
    x = f.relu(f.linear(inp, weights[:32].reshape(8,4), weights[32:40].reshape(8)))
    x = f.linear(x, weights[40:56].reshape(2,8), weights[56:].reshape(2))
    return x[0]

class implicit_br(torch.autograd.Function):

    @staticmethod
    def forward(ctx, GMM_agg_agent_logits, GMM_agg_agent):
        br = GMMAgent_nn(GMM_agg_agent.mu, GMM_agg_agent.game)
        br.x = (0.01*torch.randn(58, dtype=torch.float)).clone().detach().to(device_train)
        br.x.requires_grad = True
        opt = torch.optim.Adam([br.x], lr=0.1)
        with torch.enable_grad():
            for i in range(50):
                payoff = get_payoff_nn(br.x, GMM_agg_agent_logits, br.mu, br.game)
                ...
                opt.step()
            #calculate hessian
            h = (1/lam) * torch.autograd.functional.hessian(get_payoff_nn, (br.x, GMM_agg_agent_logits, br.mu, br.game))[0][1]
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output @ h, None

class implicit_best_responder(nn.Module):
    def __init__(self):
        super(implicit_best_responder, self).__init__()
        self.best = implicit_br.apply
    def forward(self, GMM_agg_agent_logits, GMM_agg_agent):
        br = self.best(GMM_agg_agent_logits, GMM_agg_agent)
        r = get_payoff_nn(br, GMM_agg_agent_logits, GMM_agg_agent.mu, GMM_agg_agent.game)
        return r, br